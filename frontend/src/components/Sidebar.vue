<template>
    <div>
        ...
        <div id="sidebar" :class="{'input-scrolled': contentInputScrolled, 'fullscreen': $store.state.app.fullscreen || (($store.state.mobileVersion || $store.state.app.fullscreenOnHomepage) && this.$route.path === '/'), 'hidden': $store.state.mobileVersion && this.$route.path !== '/'}">
            <svg v-if="!($store.state.mobileVersion || ($store.state.app.fullscreenOnHomepage && this.$route.path === '/')) && !($store.state.appInfo.login_required_for_create && !$store.state.user.logged_in)" id="fullscreen-button" @click="$store.state.app.fullscreen = !$store.state.app.fullscreen" width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-chevron-right" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/></svg>
            ...
            <div id="create-paste" :class="{'input-fullscreen': inputFullscreen}" style="height: calc(100% - 300px); min-height: 300px">
            ...
            <div @click="$router.push('/login-with')" id="login-required-info" v-show="($store.state.appInfo.login_required_for_create && !$store.state.user.logged_in)">
        ...
        <div id="footer">
            ...
            <a v-for="(item, key) of $store.state.appInfo.custom_footer" :href="item" :key="key" :style="{display: !isPWA() ? 'inline-block' : 'none'}"><span v-if="!isPWA() && key">{{key}}</span></a>
