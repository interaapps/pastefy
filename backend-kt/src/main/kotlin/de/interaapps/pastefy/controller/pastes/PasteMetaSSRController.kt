...
class PasteMetaSSRController(
    ...
    fun getPasteMetaSSR(@PathVariable id: String): ResponseEntity<String> {
        ...
        val title = paste.title?.trim()?.replace(Regex("\\s+"), " ")?.takeIf(String::isNotEmpty)?.let { seo.truncate(it, 120) } ?: "Paste"
        val author = if (paste.isPublic) {
            paste.userId?.let(userRepository::findById)?.orElse(null)?.uniqueName?.trim()?.takeIf(String::isNotEmpty)?.let { username ->
                Author(
                    username = username,
                    displayName = paste.userId?.let(userRepository::findById)?.orElse(null)?.name?.trim()?.takeIf(String::isNotEmpty) ?: username,
        ...
        val content = if (paste.isPublic) seoContent(pasteService.getContent(paste, withCache = false).orEmpty(), paste.key, title, author, aiInfo?.description) else ""
    ...
    private fun seoContent(content: String, pasteKey: String, title: String, author: Author?, aiDescription: String?): String {
        ...
        val aiHtml = aiDescription?.takeIf(String::isNotBlank)?.let { "<h2>Description</h2><p>${seo.escapeHtml(it)}</p>" }.orEmpty()
        return "<main id=\"seo-content\"><h1 title=\"paste-title\">${seo.escapeHtml(title)}</h1>$authorHtml<p>View and share code snippets on Pastefy.</p><pre><code>${seo.escapeHtml(preview)}</code></pre><h2>Tags</h2>$tags$aiHtml</main>"
    }
