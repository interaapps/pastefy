...
class DiscordOAuth2Provider(
    ...
    override fun authorizationUrl(callbackUrl: String, state: String): String = authorizationUrl(
        ...
        mapOf("response_type" to "code", "client_id" to clientId, "prompt" to "consent", "scope" to scopes.joinToString(" "), "redirect_uri" to callbackUrl, "state" to state),
    )

    override fun exchangeCode(code: String, callbackUrl: String): OAuth2Tokens = requireHttp().postForm(
        "https://discord.com/api/oauth2/token",
        mapOf("client_id" to clientId, "client_secret" to clientSecret, "code" to code, "grant_type" to "authorization_code", "redirect_uri" to callbackUrl, "scope" to scopes.joinToString(" ")),
    ).tokens()

    override fun loadProfile(tokens: OAuth2Tokens): OAuth2Profile {
        val data = requireHttp().get("https://discord.com/api/users/@me", mapOf("Authorization" to "Bearer ${tokens.accessToken}"))
